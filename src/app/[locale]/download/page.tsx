...
function formatChangelogContent(content: string): string {
  ...
  for (const line of lines) {
    ...
    if (trimmedLine.match(/^\*\*【.+】\*\*$/) || trimmedLine.match(/^\*\*[^*]+:\*\*$/)) {
      ...
      formattedLines.push(`<h4 class="font-semibold text-marketing-foreground mt-4 mb-2">${text}</h4>`);
      continue;
    }
    ...
    if (trimmedLine.match(/^• \*\*[^:]+:\*\*/)) {
      ...
      if (match) {
        ...
        formattedLines.push(`<li class="text-sm"><strong class="text-marketing-foreground">${title}:</strong>${processedDescription}</li>`);
      }
      continue;
    }
    ...
    if (trimmedLine.match(/^\s*-\s+/)) {
      ...
      // 处理描述中的markdown粗体语法
      const processedDescription = description.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
      formattedLines.push(`<li class="text-sm text-marketing-textSecondary">${processedDescription}</li>`);
      continue;
    }
    ...
    if (trimmedLine && !trimmedLine.startsWith('#')) {
      ...
      formattedLines.push(`<p class="text-sm text-marketing-textSecondary mb-2">${processedLine}</p>`);
    }
  }
...
export default async function DownloadPage({ params }: DownloadPageProps) {
  ...
  return (
    <>
        ...
        <section className="w-full py-16 md:py-24 lg:py-32 bg-gradient-to-br from-blue-50 to-purple-50 overflow-hidden">
          <div className="container mx-auto px-4 md:px-6">
            <div className="text-center mb-12">
              ...
              <h1 className="text-4xl font-extrabold tracking-tight text-marketing-foreground sm:text-5xl lg:text-6xl mb-6">
            ...
            <div className="grid gap-8 md:grid-cols-2 max-w-4xl mx-auto">
              {/* iOS 下载 */}
              <div className="bg-marketing-cardBg rounded-2xl p-8 shadow-lg border border-marketing-border hover:shadow-xl transition-shadow">
                <div className="text-center mb-6">
                  ...
                  <h2 className="text-2xl font-bold text-marketing-foreground mb-2">{t.download.platforms.ios.title}</h2>
                  <p className="text-marketing-textSecondary mb-4">{t.download.platforms.ios.description}</p>

                  {/* iOS 应用信息 */}
                  <div className="grid grid-cols-3 gap-4 mb-6 text-sm">
                    <div>
                      <div className="text-marketing-textSecondary">{t.download.hero.version}</div>
                      <div className="font-semibold text-marketing-foreground">{appInfo.version}</div>
                    </div>
                    <div>
                      <div className="text-marketing-textSecondary">{t.download.hero.size}</div>
                      <div className="font-semibold text-marketing-foreground">{appInfo.ios.size}</div>
                    </div>
                    <div>
                      <div className="text-marketing-textSecondary">{t.download.hero.lastUpdated}</div>
                      <div className="font-semibold text-marketing-foreground">{appInfo.lastUpdated}</div>
                    </div>
                  </div>
                </div>
              ...
              <div className="bg-marketing-cardBg rounded-2xl p-8 shadow-lg border border-marketing-border hover:shadow-xl transition-shadow">
                <div className="text-center mb-6">
                  ...
                  <h2 className="text-2xl font-bold text-marketing-foreground mb-2">{t.download.platforms.android.title}</h2>
                  <p className="text-marketing-textSecondary mb-4">{t.download.platforms.android.description}</p>
                  ...
                  <div className="grid grid-cols-3 gap-4 mb-6 text-sm">
                    ...
                    <div>
                      <div className="text-marketing-textSecondary">{t.download.hero.size}</div>
                      <div className="font-semibold text-marketing-foreground">{appInfo.android.size}</div>
                    </div>
                    <div>
                      <div className="text-marketing-textSecondary">{t.download.hero.lastUpdated}</div>
                      <div className="font-semibold text-marketing-foreground">{appInfo.lastUpdated}</div>
        ...
        {/* Changelog Section */}
        <section className="w-full py-16 md:py-24 bg-marketing-background">
          <div className="container mx-auto px-4 md:px-6">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold tracking-tight text-marketing-foreground md:text-4xl mb-4">
                {changelog.title}
              </h2>
              <p className="text-lg text-marketing-textSecondary max-w-2xl mx-auto">
            ...
            <div className="max-w-3xl mx-auto space-y-8">
              {changelog.versions.map((version, index) => (
                <div key={version.version} className="bg-marketing-cardBg rounded-2xl p-8 shadow-lg border border-marketing-border hover:shadow-xl transition-shadow">
                  <div className="flex items-start gap-4 mb-6">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        <h3 className="text-xl font-bold text-marketing-foreground">
                      ...
                      <div className="mb-4">
                        <h4 className="text-lg font-semibold text-marketing-foreground mb-2">
                          {version.title}
                        </h4>
                      </div>
                      ...
                      <div className="space-y-4">
                        <div
                          className="prose prose-sm max-w-none text-marketing-textSecondary"
            ...
            <div className="grid gap-12 lg:grid-cols-2 lg:gap-20 items-center">
                ...
                <ul className="space-y-4">
                  {t.download.requirements.items.map((requirement: any, index: number) => (
                    <li key={index} className="flex items-start gap-3">
                      ...
                      <span className="text-marketing-textSecondary">{requirement}</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="grid gap-4">
                {t.download.security.items.map((item: any, index: number) => (
                  <div key={index} className="bg-marketing-cardBg rounded-xl p-6 shadow-sm border border-marketing-border hover:shadow-md transition-shadow">
                    <div className="flex items-start gap-4">
                      <div className="text-2xl">{item.icon}</div>
                      <div>
                        <h3 className="font-bold text-marketing-foreground mb-2">{item.title}</h3>
                        <p className="text-sm text-marketing-textSecondary">{item.description}</p>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="max-w-4xl mx-auto">
              <div className="grid gap-8 md:grid-cols-2">
                {t.download.installation.steps.map((step: any, index: number) => (
                  <div key={index} className="bg-marketing-cardBg rounded-2xl p-6 shadow-sm border border-marketing-border">
                    <div className="flex items-center gap-4 mb-4">
                      <div className="flex items-center justify-center w-10 h-10 bg-green-100 rounded-full font-bold text-green-600">
                        {step.number}
                      </div>
                      <h3 className="text-xl font-bold text-marketing-foreground">{step.title}</h3>
                    </div>
            ...
            <div className="max-w-4xl mx-auto space-y-6">
              {t.download.faq.items.map((faq: any, index: number) => (
                <div key={index} className="bg-marketing-cardBg rounded-xl p-6 shadow-sm border border-marketing-border">
                  <h3 className="font-bold text-marketing-foreground mb-3 text-lg">{faq.question}</h3>
                  <p className="text-marketing-textSecondary">{faq.answer}</p>
                </div>
              ))}
            </div>
          </div>
